...
export async function POST(request: Request) {
  try {
    ...
    if(action==="prepare") {
      ...
      const code=scriptTemplate.replace("__TRACE_CONFIG__",JSON.stringify({...locations,secret:current.secret,columns:tableColumns},null,2));
    ...
    if(action==="connect") {
      ...
      const health=await bridgeCall<{version:number;spreadsheetId:string;folderId:string;ownerEmail:string}>("health",{},candidate);
    ...
    if(action==="start") {
      ...
      await bridgeCall("health",{},config);
