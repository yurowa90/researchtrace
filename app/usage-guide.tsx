...
export function UsageGuide({ data, students, onNavigate, onAddClass, onAddStudents }: GuideProps) {
  ...
  const steps = staff ? [
    ...
    { title: "학생부 원본을 보관합니다", text: "2학년은 1학년, 3학년은 1·2학년 학생부를 기본으로 등록합니다. 현재 학년 내용이 있으면 추가하고, 한 파일에 여러 학년이 들어 있으면 모두 선택합니다. 학생 카드의 원본 추가를 누르면 대상 학생과 필요한 학년이 미리 선택됩니다. 학년도는 학급 정보를 기준으로 제안되므로 유급·전입 등 이력이 있으면 직접 수정합니다.", button: "학생부 원본", view: "records" as ViewId, icon: FileText },
    { title: "Work에서 분석합니다", text: "Work 결과 메뉴에서 분석 요청문을 복사하고 JSON 예시를 내려받습니다. 공용 평가 자료실에서 관리자가 등록한 모집요강·평가 기준을 선택해 저장합니다. 자료 묶음을 한 번 받아 Work에서 재사용하고, 학생별로 학생부와 요청문을 제공합니다. 사이트 원본은 Work로 자동 전송되지 않습니다.", button: "Work 결과", view: "import" as ViewId, icon: FileJson },
    ...
    { title: "본인 계정 연결을 확인합니다", text: "학교에 등록한 이메일과 같은 계정으로 로그인합니다. 승인 대기이거나 본인 자료가 보이지 않으면 담임에게 계정 연결 상태를 확인받습니다.", button: "내 현황", view: "overview" as ViewId, icon: Users },
    ...
    { title: "근거와 원본을 대조합니다", text: "교과·성적과 역량 점검은 원본에 근거한 참고 자료입니다. 잘못된 내용은 담임에게 정정을 요청합니다. 학생 계정에서는 수정·업로드할 수 없습니다.", button: "교과·성적", view: "academics" as ViewId, icon: FileText },
  ];
  return <div className="space-y-6">
    ...
    <section className="rounded-2xl border border-[#dfe6ef] bg-white px-5"><h2 className="pt-5 font-bold">자주 막히는 부분</h2><Accordion type="single" collapsible>
      ...
      <AccordionItem value="evaluation"><AccordionTrigger>역량 점검 결과가 없거나 점수가 확정적이지 않습니다.</AccordionTrigger><AccordionContent className="text-base leading-7 text-[#596d85]">공용 평가 자료실에서 대학·전형·학년도에 맞는 자료를 선택하고 실제 파일을 Work에 처음 한 번 전달하세요. 해당 기준을 연결한 결과를 가져오면 역량 점검이 나타납니다. 점수는 분석 보조값이며 대학의 공식 평가점수나 합격 확률이 아닙니다.</AccordionContent></AccordionItem>
