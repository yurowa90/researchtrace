...
export function RecordsView({ data, students, staff, selectedStudentId, onSelect, onUploaded }: {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-end justify-between gap-4"><div><h1 className="text-3xl font-bold tracking-tight">학생부 원본 관리</h1><p className="mt-3 text-base leading-7 text-[#65768b]">2학년은 1학년, 3학년은 1·2학년 원본이 기본입니다. 현재 학년 기록이 있으면 함께 등록해 추가 분석할 수 있습니다.</p></div>{staff && <Button disabled={!students.length} className="bg-[#173a73]" onClick={() => beginUpload(selectedStudentId ?? students[0]?.id)}><Upload />학생부 업로드</Button>}</div>
