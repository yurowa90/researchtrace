...
export function ResearchPortal() {
  ...
  const content: Record<ViewId, React.ReactNode> = {
    ...
    settings: <>{admin && <StorageSettings onChanged={load} />}<SchoolSettings data={data} students={visibleStudents} staff={staff} admin={admin} busy={busy} onAddClass={() => setClassDialog(true)} onAddStudent={() => setStudentDialog(true)} onBulkAddStudent={() => setBulkStudentDialog(true)} onStatus={async (studentId, status, graduatedYear) => { await execute({ action: "updateStudentStatus", studentId, status, graduatedYear }, status === "graduated" ? "졸업생으로 보존 처리했습니다." : "재학 상태로 복원했습니다.").catch(() => undefined); }} onApproveStudent={async (userId, studentId) => { await execute({ action: "approveUser", userId, studentId }, "학생 계정을 연결했습니다.").catch(() => undefined); }} onApproveTeacher={async (userId) => { await execute({ action: "approveTeacher", userId }, "담임 계정을 승인했습니다.").catch(() => undefined); }} onAssignClassTeacher={async (targetClassId, teacherId) => { await execute({ action: "assignClassTeacher", classId: targetClassId, teacherId }, "학급 담당자를 변경했습니다.").catch(() => undefined); }} /></>,
  };

  return <div className="min-h-screen bg-[#f4f7fb] text-[#102342]">
    <Toaster position="top-right" richColors />
    <aside className="fixed inset-y-0 left-0 z-30 hidden w-[252px] flex-col bg-[#102b55] px-4 py-5 text-white lg:flex"><Brand /><div className="mt-8 px-3 text-[11px] font-semibold uppercase tracking-[.14em] text-blue-200/70">Student intelligence</div><nav className="mt-3 space-y-1" aria-label="주요 메뉴">{navItems.filter((item) => !item.staffOnly || staff).map((item) => <button key={item.id} type="button" onClick={() => setView(item.id)} className={`flex w-full items-center gap-3 rounded-xl px-3 py-2.5 text-left text-[14px] transition ${view === item.id ? "bg-white text-[#153b75] shadow-sm" : "text-blue-50/80 hover:bg-white/10 hover:text-white"}`}><item.icon className="size-[17px]" />{item.label}</button>)}</nav><div className="mt-auto rounded-2xl border border-white/10 bg-white/5 p-4"><div className="flex items-center gap-2 text-sm font-semibold"><ShieldCheck className="size-4 text-[#77e0c2]" /> 3단계 열람 권한</div><p className="mt-2 text-xs leading-5 text-blue-100/70">학생은 본인, 담임은 담당 학급, 관리자는 학교 전체와 졸업생을 조회합니다.</p></div></aside>
