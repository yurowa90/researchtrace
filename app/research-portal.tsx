...
export function ResearchPortal() {
  ...
  React.useEffect(() => {
    ...
    if (navItems.some(item => item.id === requested)) setView(requested as ViewId);
    const studentId = Number(params.get("student"));
    if (Number.isSafeInteger(studentId) && studentId > 0) setSelectedStudentId(studentId);
  }, []);
  ...
  const load = React.useCallback(async () => {
    try {
      ...
      const requestedStudent = Number(new URLSearchParams(window.location.search).get("student"));
      if (body.students.some(s => s.id === requestedStudent && s.isExample)) setShowExamples(true);
  ...
  const content: Record<ViewId, React.ReactNode> = {
    ...
    settings: <>{admin && <><SchoolAccessPanel /><SchoolBaselinePanel /><SchoolBackupPanel /><StorageSettings onChanged={load} /></>}<SchoolSettings data={data} students={visibleStudents} staff={staff} admin={admin} busy={busy} onAddClass={() => setClassDialog(true)} onAddStudent={() => setStudentDialog(true)} onBulkAddStudent={() => setBulkStudentDialog(true)} onStatus={async (studentId, status, graduatedYear) => { await execute({ action: "updateStudentStatus", studentId, status, graduatedYear }, status === "graduated" ? "졸업생으로 보존 처리했습니다." : "재학 상태로 복원했습니다.").catch(() => undefined); }} onApproveStudent={async (userId, studentId) => { await execute({ action: "approveUser", userId, studentId }, "학생 계정을 연결했습니다.").catch(() => undefined); }} onApproveTeacher={async (userId) => { await execute({ action: "approveTeacher", userId }, "담임 계정을 승인했습니다.").catch(() => undefined); }} onAssignClassTeacher={async (targetClassId, teacherId) => { await execute({ action: "assignClassTeacher", classId: targetClassId, teacherId }, "학급 담당자를 변경했습니다.").catch(() => undefined); }} /></>,
  };

  return <div className="min-h-screen bg-[#f4f7fb] text-[#102342]">
    <Toaster position="top-right" richColors />
    {admin && <a href="/" className="fixed bottom-4 right-4 z-40 rounded-xl border border-[#c5d5e7] bg-white px-4 py-3 text-sm font-semibold text-[#173a73] shadow-lg">관리자 운영 화면</a>}
