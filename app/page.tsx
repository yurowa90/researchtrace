...
export async function generateMetadata() { return {title:schoolSite().mode==="student"?"TRACE | 나의 탐구 기록":schoolSite().mode==="teacher"?"TRACE | 교사 지도실":"TRACE | 학교 관리자"}; }

export default async function Home() {
  const user = await getChatGPTUser();
  const allowLocalPreview = process.env.NODE_ENV !== "production";
  const teacher = schoolSite().mode === "teacher", student = schoolSite().mode === "student";
    ...
    return (
      <main className="grid min-h-screen place-items-center bg-[#f5f7fb] px-6">
        <section className="w-full max-w-md rounded-[28px] border border-[#dfe5ee] bg-white p-8 shadow-[0_24px_80px_rgba(18,40,76,0.10)]">
          ...
          <h1 className="text-2xl font-bold tracking-[-0.03em] text-[#102342]">{student ? "나의 탐구 기록" : teacher ? "교사 지도실" : "학교 관리자"}</h1>
          <p className="mt-3 leading-7 text-[#5c6d83]">
            {student ? "나의 질문과 실행 기록, 교과 성적과 연구 흐름을 살펴봅니다. 학교에서 확인한 학생 계정으로 로그인하세요." : teacher ? "담당 학급의 학생부와 연구 기록을 확인하고 상담·피드백을 이어갑니다. 승인된 교사 계정으로 로그인하세요." : "학생·학급·계정과 누적 자료를 관리합니다. 승인된 학교 관리자 계정으로 로그인하세요."}
          </p>
  ...
  return student ? <StudentPortal adminUrl={portalLink("TRACE_ADMIN_PORTAL_URL")} /> : teacher ? <TeacherPortal adminUrl={portalLink("TRACE_ADMIN_PORTAL_URL")} /> : <AdminPortal />;
}
