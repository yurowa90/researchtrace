    ...
    <div className="lg:pl-60">
      ...
      <main className="mx-auto max-w-[1440px] space-y-6 px-4 py-6 sm:px-8 sm:py-8">
        ...
        {section==="overview"&&<>
          ...
          <section className="rounded-2xl border border-[#dfe6ef] bg-white p-5"><h2 className="text-lg font-bold">자료 보관과 사이트 연결</h2><p className="mt-4 font-semibold">{report.storage==="google"?"Google Drive·Sheets 사용 중":report.storage==="migrating"?"구글 저장소로 이전 중":"기존 사이트 저장소 사용 중"}</p><p className="mt-3 text-sm leading-6 text-[#65768b]">{report.storage==="google"?"같은 학교 저장소를 연결할 준비가 되어 있습니다. 학생·교사 사이트의 공통 연결 설정과 계정 승인을 완료하면 같은 자료를 조회합니다.":"구글 연결·자료 이전을 완료하면 학생·교사 사이트에서도 같은 기록을 사용합니다. 현재 자료는 기존 저장소에 보존됩니다."}</p><Button className="mt-4" variant="outline" onClick={()=>navigate("storage")}>저장 위치·백업 확인</Button>{report.teacherUrl&&<Button asChild variant="outline" className="ml-2 mt-4"><a href={report.teacherUrl} target="_blank" rel="noreferrer">교사 사이트 열기<ArrowUpRight className="size-4"/></a></Button>}{report.studentUrl&&<Button asChild variant="outline" className="ml-2 mt-4"><a href={report.studentUrl} target="_blank" rel="noreferrer">학생 사이트 열기<ArrowUpRight className="size-4"/></a></Button>}<div className="mt-5 border-t pt-4 text-sm leading-6 text-[#65768b]"><p>공용 자료 {data.referenceMaterials.filter(r=>r.status==="active").length}개 · 등록 학급 {data.classes.length}개</p><p>AI 분석은 Work에서 진행하고 결과를 가져옵니다.</p></div></section></div>
