...
export function referenceManifest(materials: ReferenceMaterial[]) {
  return materials.map((item) => ({ id: referenceKey(item.id), title: item.title, institution: item.institution, admissionsYear: item.admissionsYear, admissionTrack: item.admissionTrack, category: item.category, note: item.note, fileName: referenceFileName(item), sha256: item.sha256 }));
}
export function referenceContextText(materials: ReferenceMaterial[]) {
...
자료 목록:
${JSON.stringify(referenceManifest(materials), null, 2)}`;
}
export function libraryReferenceIssues(profile: ProfileImport, materials: ReferenceMaterial[]): string[] {
  const issues: string[] = [];
